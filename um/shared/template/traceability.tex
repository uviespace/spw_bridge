...
\newcommand{\meetsird}[1]{% (handles comma separated list of IRD requirements)
  \xintListWithSep{, }{\xintApply{ \meetsthisird}{\xintCSVtoList{#1}}}%
}
...
\newcommand{\design}[5]{%
	...
	\begin{tcolorbox}[enhanced, notitle, clip upper,%
		...
		tabularx={%
						  p{0.15\columnwidth}%
				>{\arraybackslash\raggedright}p{0.12\columnwidth}%
				>{\arraybackslash}X%
			}%
		]%
	%
...
\newcommand{\exportirdrequirements}{%
\message{reqstart exportirdrequirements}
\AtEndDocument{%
	\newwrite\irdreqfile
	\immediate\openout\irdreqfile=requirements_ird.list
	\immediate\write\irdreqfile{\string\renewcommand{\string\irdrequirements}{\requirements}}
	\immediate\closeout\irdreqfile
}
\message{reqend exportirdrequirements}
}%
...
%
\newcommand*\rot[1]{\multicolumn{1}{C}{\makebox[0.35cm][c]{\rotatebox{90}{##1}}}}%
%
	\begingroup % localize the following settings
	\setlength{\LTleft}{0pt minus 1000pt}
	\setlength{\LTright}{0pt minus 1000pt}
	%\begin{table}[h]
	\newcolumntype{C}{>{\centering\arraybackslash} m{0.35cm}}%
	...
	\xintFor* ##1 in \designs \do {%
		&  \rot{\fontspec{MyriadPro}\fontsize{7}{8}\ref{##1}\,\,\normalfont}%
	} \\%
	\endhead
	...
	%	\if T\theresult moopp\hypersetup{filecolor=uvie-burgundy}\fi
		...
		\fontspec{MyriadPro}\fontsize{7}{8}\ref{REQ-##1}\normalfont %
...
\newcommand*\irdtraceabilitymatrix{%
%
	\newcommand{\numitems}[1]{{\expandafter\xintLength\expandafter{##1}}}%
	\newcommand*\OK{\ding{51}}%
	\newcounter{numirddesigns}%
	\setcounter{numirddesigns}{\numitems{\designs}}%
	\stepcounter{numirddesigns}%
%
\newcommand*\rot[1]{\multicolumn{1}{C}{\makebox[0.35cm][c]{\rotatebox{90}{##1}}}}%
%
	\begingroup % localize the following settings
	...
	\rowcolors{1}{uvie-gray!25}{white}%
	\setlength{\tabcolsep}{2.5pt}%
	\begin{longtable}{@{}c*{\value{numirddesigns}}CC@{}} %+1 linespacing column
	\xintFor* ##1 in \designs \do {%
		&  \rot{\fontspec{MyriadPro}\fontsize{7}{8}\ref{##1}\,\,\normalfont}%
	} \\%
	\endhead
	\xintFor* ##1 in \irdrequirements \do {%
		\fontspec{MyriadPro}\fontsize{7}{8}\ref{IRD-##1}\normalfont %
		\hypersetup{filecolor=uvie-blue}
		\xintFor* ##2 in \designs \do {%
			&%
			\xintFor* ##3 in {\designswithreq{##1}} \do {%
