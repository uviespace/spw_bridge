...
\begin{versionhistory}
  ...
  \vhEntry{1.1}{10.09.2026}{AL}{add monitor mode bridging two SpaceWire channels with RMAP decoding}
\end{versionhistory}
...
\begin{tabu} to \linewidth {@{}ll@{}}
	...
	\texttt{spw.c}		& SpaceWire subsystem: STAR API device and link handling \\
	\texttt{kbd.c}, \texttt{kbd.h}& keyboard monitor: toggles the PUS debug printout \\
...
\gls{RMAP} is reported as such. No \gls{PUS} interpretation is performed on
the monitor path, and packets are never modified by the decoding.
...
{\ttfamily not an RMAP packet} line and only its payload is dumped. The payload
itself is formatted as in the PUS printout (Section~\ref{sec:pusdbg}): hex
octets, sixteen per line, with an empty line closing the dump.
...
resubmitting the command line. The keyboard control is handled by a dedicated
monitor thread and requires \texttt{stdin} to be connected to a terminal; if
the bridge runs in the background or with redirected input, the toggle is not
available.
