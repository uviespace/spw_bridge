...
\begin{versionhistory}
  ...
  \vhEntry{1.1}{10.09.2026}{AL}{add monitor mode bridging two SpaceWire channels with RMAP decoding, payload suppression, a short debug form and sequence checking in the debug printout}
\end{versionhistory}
...
Section~\ref{sec:puscrc}). The next expected value is tracked per APID and
always advances to the received value plus one (with wrap-around at 14 bits);
a previously unseen APID starts tracking at the received value plus one. A
sequence mismatch is reported together with the expected and received counter
values in the PUS debug printout (Section~\ref{sec:pusdbg}) and is only shown
when \texttt{-D} is active.\\
